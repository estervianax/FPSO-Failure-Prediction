...
class DataExplorator:
    ...
    def plot_failure_rate_by_category(self, df, category_column: str):
        ...
        ax = sns.barplot(
            data=plot_data, x=category_column, y="Failure Rate (%)", palette="magma"
        )
        ...
        plt.title(
            f'Failure Rate by {category_column.replace("_", " ").title()}', fontsize=14
        )
    ...
    def plot_column_distribution(self, df, column: str):
        ...
        plt.figure(figsize=(10, 6))
        sns.barplot(
            data=df, x=column, y="count", palette=[color_map[x] for x in df.index]
        )
    ...
    def get_top_preset_combinations_before_failure(self, df):
        ...
        """
        presets_before_failure = df[(df["fail_next"] == True) & (df["fail"] == 0)][
            "preset_combined"
        ]

        preset_percent = presets_before_failure.value_counts(
            normalize=True
    ...
    def evaluate_preset_transition_failure_impact(
        self, df, preset_combined_seq_1, preset_combined_seq_2
    ):
        """
        ...
        """
        df_copy = df.copy()
        df_copy["combo_lag1"] = df_copy["preset_combined"].shift(1)
        mask = (
            (df_copy["combo_lag1"] == preset_combined_seq_1)
            & (df_copy["preset_combined"] == preset_combined_seq_2)
            & (df_copy["fail"] == 1)
        )
        count = mask.sum()
        total_fails = df_copy["fail"].sum()
        percentage = round(100 * count / total_fails, 2)

        print(
            f"Sequence {preset_combined_seq_1} → {preset_combined_seq_2} → failure occurred {count} times."
        )
    ...
    def calculate_overall_and_before_failure_variable_mean(self, df, column):
        """
    ...
    def plot_variables_time_series_with_failures(self, df, variable: str):
        ...
        plt.scatter(
            failures.index, failures[variable], color="#d3436e", label="Fail", zorder=5
        )
        mean_value = df[variable].mean()
        plt.axhline(
            mean_value, color="#982d80", linestyle="--", label=f"Mean: {mean_value:.2f}"
        )
    ...
    def average_failure_duration_cycles(self, df):
        ...
        df_copy["fail_group"] = (
            df_copy["fail_continued"] != df_copy["fail_continued"].shift()
        ).cumsum()
