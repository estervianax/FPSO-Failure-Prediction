class DataPreprocessor(object):
    ...
    def transform(self):
        ...
        df_transformed, categorical_cols, numerical_cols = self.__separate_columns_groups(df_transformed)
        return df_transformed, categorical_cols, numerical_cols

    def create_time_based_columns(self,df,numerical_features_list,all_features_list,set_window=5):
        df_times_based= self.__create_rolling_mean_columns(df,numerical_features_list,set_window)
        ...
        df_times_based, times_based_categorical_cols, times_based_numerical_cols = self.__separate_columns_groups(df_times_based)
    ...
    def __create_rolling_mean_columns(self,df,numerical_columns_list,set_window):
        for column in numerical_columns_list:
            df[f'{column}_rolling_{set_window}'] = df[column].rolling(window=set_window).mean()
        return df
    ...
    def __separate_columns_groups(self,df):
        ...
        for col in df.columns:
            if col.startswith('preset') or col.startswith('fail'):
                df[col] = df[col].astype('category')
                categorical_cols.append(col)
            elif (col.startswith('temperature') or 
                col.startswith('pressure') or 
                col.startswith('vibration')):
